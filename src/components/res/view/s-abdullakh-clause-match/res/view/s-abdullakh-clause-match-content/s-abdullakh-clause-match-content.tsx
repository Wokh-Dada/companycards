...
@Component({
  tag: 's-abdullakh-clause-match-content',
  styleUrl: 's-abdullakh-clause-match-content.css',
  shadow: false,
})
export class SAbdullakhClauseMatchContent implements ComponentInterface {
  ...
  render() {
    return (
      <div class="clause_match_block_contest">
        <div class="block_name" onClick={() => this.clickOnClauseMatch.emit(this.clauseMatch.blockName)}>
          {this.clauseMatch.blockName}
        </div>
        <blockquote class="">
          <div class="clause_match_block_left_block" onClick={() => this.clickOnClauseMatch.emit(event)}>
            ...
            <h3 class="" onClick={() => this.clickOnClauseMatch.emit(this.clauseMatch.title)}>
              {this.clauseMatch.title}
            </h3>
            <div data-testid="test-quote" class="clause_match_left_block_text" onClick={() => this.clickOnClauseMatch.emit(this.clauseMatch.blockContent)}>
              {this.clauseMatch.blockContent}
            </div>
            <div class="clause_match_left_block_author" onClick={() => this.clickOnClauseMatch.emit(this.clauseMatch.company)}>
              <span class="pr-2" onClick={() => this.clickOnClauseMatch.emit(this.clauseMatch.author)}>
